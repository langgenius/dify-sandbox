...
func init() {
	...
	if err != nil {
		...
		if err != nil {
			log.Error("failed to create user (proceeding anyway): %v, %v", err, string(output))
		}
	}
	...
	if err != nil {
		log.Error("failed to get gid of user: %v", err)
		static.SANDBOX_GROUP_ID = 1000 // Default fallback
	} else {
		static.SANDBOX_GROUP_ID, err = strconv.Atoi(strings.TrimSpace(string(gid)))
		if err != nil {
			log.Error("failed to convert gid: %v", err)
		}
	}
}
