...
func (p *UvRunner) Run(
	...
	cmd.Env = []string{"UV_CACHE_DIR=" + LIB_PATH + "/root/.cache/uv"}
	cmd.Dir = LIB_PATH
...
func (p *UvRunner) InitializeEnvironment(code string, preload string, options *types.RunnerOptions) (string, string, error) {
	...
	dependencies := []string{}
	if options != nil && options.Dependenchies != nil {
		for _, dep := range options.Dependenchies {
			if dep.Version == "" {
				dependencies = append(dependencies, fmt.Sprintf("\"%s\"", dep.Name))
			} else {
	...
	// if there are dependencies, add the package path to the script
	if len(dependencies) != 0 {
		code = fmt.Sprintf("import sys;import sysconfig;sys.path.insert(0, sysconfig.get_paths()[\"purelib\"].replace(\"%s\", \"\"))\n%s", LIB_PATH, code)
	}
